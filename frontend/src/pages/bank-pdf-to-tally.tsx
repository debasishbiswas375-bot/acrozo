...
function sanitizeTransaction(tx: Transaction): Transaction {
  const cleanName = (name: string): string => {
    ...
    if (nameUpper.includes("SUSPENSE")) {
      return "Suspense A/c";
    }
...
function rebuildXML(
  ...
  transactions.forEach((tx, i) => {
    ...
    const guid = `ZAIZ-${cleanDate}-${i}-${Math.random().toString(36).slice(2, 10)}`;
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function BankPdfToTally() {
  ...
  const handleDownloadXml = async () => {
    ...
    // 2. Record the download in history (no server-side file stored — Render has no uploads folder)
    ...
    try {
      ...
      if (res.ok) {
        ...
        setUploadedUrl(url ?? null); // url will be null — files are not stored server-side
      }
    } catch (e) {
      ...
      <main className="max-w-[95%] mx-auto px-4 py-8 pt-20">
        ...
        <div className="glass-card rounded-3xl shadow-xl p-6 md:p-10 relative overflow-hidden bg-white/70 dark:bg-gray-900/60 backdrop-blur-xl border border-indigo-100/20">
          ...
          <div className="text-center mb-8 relative">
            ...
            >
              <img src="/logo.png" alt="ZaiZ Logo" className="w-10 h-10 object-contain invert dark:invert-0" />
            </div>
            <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Bank PDF → Tally XML</h1>
            <p className="text-muted-foreground text-sm mt-2 max-w-xl mx-auto leading-relaxed">
              Upload your bank statement PDF. ZaiZ extracts every transaction and generates a <strong>single-entry Tally XML</strong> — one bank ledger line per voucher. Tally handles the contra side automatically.
            </p>
          </div>
                ...
                {file ? (
                  <div className="flex flex-col items-center justify-center space-y-4 py-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-1">{(file.size / 1024).toFixed(1)} KB • PDF Statement</p>
                    </div>
                    <button
                  ...
                  <div className="space-y-4 py-4">
                    ...
                    <div>
                      <p className="font-bold text-gray-700 dark:text-gray-300 text-sm group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">Drop your bank statement PDF here</p>
                      <p className="text-xs text-gray-400 mt-1">or click to browse from local computer</p>
                    </div>
                  </div>
                )}
              </div>
                  ...
                  <div className="space-y-8">
                    ...
                    {taskStatus.status === "error" || error ? (
                      <div className="space-y-6 text-center transition-all duration-500">
                        <div
                          className="inline-flex items-center justify-center w-16 h-16 bg-red-50 dark:bg-red-950/30 rounded-full border border-red-200 dark:border-red-900/50 shadow-lg shadow-red-500/10 mb-2 animate-bounce"
                          style={{ animationDuration: "3s" }}
                        >
                          <AlertCircle className="w-8 h-8 text-red-500 dark:text-red-400" />
                        </div>
                        <div>
                          <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Conversion Pipeline Failed</h2>
                          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1.5 max-w-md mx-auto">
                            The ZaiZ engine encountered a processing error while handling your document. Refer to the logs below for diagnosis.
                          </p>
                        </div>
                      ...
                      <div className="space-y-7">
                        ...
                        {taskStatus.status === "pending" && taskStatus.queue_position !== undefined && taskStatus.queue_position > 0 ? (
                          <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-xl p-4 text-center mb-6 shadow-sm">
                            <h3 className="text-amber-800 dark:text-amber-300 font-bold flex items-center justify-center gap-2">
                              <Loader2 className="w-5 h-5 animate-spin" />
                              Waiting in Queue
                            </h3>
                            <p className="text-amber-700 dark:text-amber-400 mt-1 text-sm font-semibold">
                        ...
                        <div className="flex flex-col items-center text-center">
                          ...
                          <div className="relative flex items-center justify-center w-24 h-24 rounded-full bg-gradient-to-tr from-indigo-50 to-violet-50 dark:from-indigo-950/20 dark:to-violet-950/20 shadow-inner border border-indigo-100/30 mb-4">
                            <div className="absolute inset-1 rounded-full bg-white dark:bg-gray-900 flex flex-col items-center justify-center">
                              {!pageDisplayInfo.showPagesToggle || elapsedSec % 2 === 0 ? (
                                ...
                                <div className="text-center">
                                  <span className="text-base font-extrabold text-indigo-600 dark:text-indigo-400 block leading-none">
                                    {pageDisplayInfo.text}
                                  </span>
                                  <span className="text-[9px] text-gray-400 font-medium block mt-0.5 uppercase tracking-wider leading-none">
                          ...
                          <div className="mb-4">
                            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">
                              {taskStatus.progress >= 95 ? "Finalizing Books..." : "Processing Document"}
                            </h3>
                            <p className="text-xs text-gray-400 mt-0.5">Please keep this window open while the AI parses data</p>
                          </div>

                          <div className="flex justify-center pt-1 mb-4">
                            <button
                              onClick={reset}
                              className="bg-red-50 dark:bg-red-950/25 hover:bg-red-100 dark:hover:bg-red-950/40 text-red-600 dark:text-red-400 border border-red-200/40 dark:border-red-900/40 font-semibold text-xs px-4.5 py-2 rounded-xl transition-all shadow-sm flex items-center gap-1.5 cursor-pointer active:scale-95"
                            >
                          ...
                          {[
                            ...
                            <div
                              ...
                              className={`rounded-2xl p-3.5 flex flex-col items-center gap-2.5 transition-all duration-300 border backdrop-blur-sm
                                ${s.done
                                  ? "border-emerald-200/50 bg-emerald-500/5 dark:bg-emerald-950/10 text-emerald-700 dark:text-emerald-400 shadow-sm"
                                  : "border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/40 text-gray-400"}`}
                            >
                              {s.done ? (
                                <div className="w-7 h-7 rounded-full bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center border border-emerald-200/30">
                                  <Check className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                                </div>
                              ) : (
                                <div className="w-7 h-7 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center border border-gray-200/10">
            ...
            <div className="space-y-5">
              ...
              {uploadedUrl && (
                <div className="flex items-center gap-2 px-4 py-2.5 bg-green-50 border border-green-200 rounded-xl text-xs text-green-700">
                  ...
                  <span>File saved to cloud. You can download it anytime from <strong>History</strong>.</span>
                </div>
              )}
